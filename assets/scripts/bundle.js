...
// ASYNC (AJAX etc.)
function async(name) {
  if (name in async_modules) async_modules[name] += 1;else async_modules[name] = 1;
}
...
function async_check() {
  for (var i in async_modules) {
    if (async_modules[i] != 0) return;
  }
  if (async_done_callback) async_done_callback();
}
...
window.update_run = function update_run(arg) {
  if (!UPDATE && arg) requestAnimationFrame(update);
  UPDATE = arg;
};
...
},{"./aircraft":3,"./airline":4,"./airport":5,"./animation":6,"./base":7,"./canvas":8,"./game":9,"./get":10,"./input":11,"./load":12,"./parser":14,"./speech":15,"./tutorial":16,"./ui":17,"./util":18,"fiber":1,"jquery":2}],14:[function(require,module,exports){
"use strict";
