 ...
 * @extends Fiber
 */
var AircraftConflict = _fiber2.default.extend(function () {
  return {
    init: function init(first, second) {
    ...
    hasConflict: function hasConflict() {
      for (var i in this.conflicts) {
        if (this.conflicts[i]) return true;
      }
      return false;
    },
     ...
     */
    hasViolation: function hasViolation() {
      for (var i in this.violations) {
        if (this.violations[i]) return true;
      }
      return false;
    },
    ...
    update: function update() {
      ...
      if (this.aircraft[0].altitude - airportElevation < 990 || this.aircraft[1].altitude - airportElevation < 990) return;

      // Ignore aircraft in the first minute of their flight
      if (game_time() - this.aircraft[0].takeoffTime < 60 || game_time() - this.aircraft[0].takeoffTime < 60) {
        return;
      }
     ...
     */
    checkRunwayCollision: function checkRunwayCollision() {
      ...
      if (!this.aircraft[0].isTaxiing() && !this.aircraft[1].isTaxiing() && this.aircraft[0].rwy_dep != null && this.aircraft[0].rwy_dep != this.aircraft[1].rwy_dep && airport.getRunway(this.aircraft[1].rwy_dep) === airport.getRunway(this.aircraft[0].rwy_dep) && this.distance < 10) {
        if (!this.conflicts.runwayCollision) {
          ...
          ui_log(true, this.aircraft[0].getCallsign() + " appears on a collision course with " + this.aircraft[1].getCallsign() + " on the same runway");
     ...
     */
    checkProximity: function checkProximity() {
      ...
      if (a1.isPrecisionGuided() && a2.isPrecisionGuided() && a1.rwy_arr != a2.rwy_arr) {
        // both are following different instrument approaches
      ...
      // Update Conflicts
      if (conflict) this.conflicts.proximityConflict = true;else this.conflicts.proximityConflict = false;
      if (violation) this.violations.proximityViolation = true;else this.violations.proximityViolation = false;
    }
  };
});
...
},{"fiber":1}],21:[function(require,module,exports){
'use strict';
 ...
 * Manage current and future aircraft waypoints
  ...
  * @extends Fiber
 */
var AircraftFlightManagementSystem = _fiber2.default.extend(function () {
  return {
    init: function init(options) {
      ...
      if (options.aircraft.category == "arrival") this.prependLeg({ route: "KDBG" });else if (options.aircraft.category == "departure") this.prependLeg({ route: airport_get().icao });
     ...
     */
    prependLeg: function prependLeg(data) {
     ...
     */
    insertWaypointHere: function insertWaypointHere(data) {
      var prev = this.currentWaypoint();
      this.currentLeg().waypoints.splice(this.current[1], 0, new _Waypoint2.default(data, this));
      this.update_fp_route();
     ...
     */
    insertLeg: function insertLeg(data) {
     ...
     */
    insertLegHere: function insertLegHere(data) {
     ...
     */
    appendLeg: function appendLeg(data) {
     ...
     */
    appendWaypoint: function appendWaypoint(data) {
      this.currentLeg().waypoints.splice(this.current[1] + 1, 0, new _Waypoint2.default(data, this));
      this.update_fp_route();
    },
     ...
     */
    skipToFix: function skipToFix(name) {
      var prev = this.currentWaypoint();
     ...
     */
    update_fp_route: function update_fp_route() {
      var r = [];
      // TODO: simplify this
      for (var l in this.legs) {
        if (!this.legs[l].type) continue;else if (this.legs[l].type == 'sid') {
          ...
          if (r[r.length - 1] != this.legs[l].route.split('.')[0]) r.push(this.legs[l].route.split('.')[0]); // airway entry fix
     ...
     */
    followCheck: function followCheck() {
     ...
     */
    followClear: function followClear() {
      this.following = {
        sid: null,
        star: null,
        iap: null,
     ...
     */
    followApproach: function followApproach(type, rwy, variant) {
     ...
     */
    followSID: function followSID(route) {
      for (var i = 0; i < this.legs.length; i++) {
     ...
     */
    followSTAR: function followSTAR(route) {
      for (var i = 0; i < this.legs.length; i++) {
     ...
     */
    formatRoute: function formatRoute(data) {
     ...
     */
    customRoute: function customRoute(route, fullRouteClearance) {
      var legs = [];
      var curr = this.currentWaypoint(); // save the current waypoint
      for (var i = 0; i < route.length; i++) {
        if (route[i].split('.').length == 1) {
          // just a fix/navaid
          legs.push(new _Leg2.default({ type: "fix", route: route[i] }, this));
          ...
          if (Object.keys(airport_get().sids).indexOf(pieces[1]) > -1) {
            // it's a SID!
            legs.push(new _Leg2.default({ type: "sid", route: route[i] }, this));
          } else if (Object.keys(airport_get().stars).indexOf(pieces[1]) > -1) {
            // it's a STAR!
            legs.push(new _Leg2.default({ type: "star", route: route[i] }, this));
          } else if (Object.keys(airport_get().airways).indexOf(pieces[1]) > -1) {
            // it's an airway!
            legs.push(new _Leg2.default({ type: "awy", route: route[i] }, this));
          }
        } else {
     ...
     */
    clearedAsFiled: function clearedAsFiled() {
     ...
     */
    climbViaSID: function climbViaSID() {
     ...
     */
    descendViaSTAR: function descendViaSTAR() {
      // Find the STAR leg
      var wp, legIndex;
      for (var l in this.legs) {
     ...
     */
    hasWaypoint: function hasWaypoint(name) {
      for (var i = 0; i < this.legs.length; i++) {
        for (var j = 0; j < this.legs[i].waypoints.length; j++) {
     ...
     */
    indexOfWaypoint: function indexOfWaypoint(fix) {
      var wp = 0;
      for (var l = 0; l < this.legs.length; l++) {
        for (var w = 0; w < this.legs[l].waypoints.length; w++) {
     ...
     *           lw: "position-of-fix-in-leg-wp-matrix"}
     */
    indexOfCurrentWaypoint: function indexOfCurrentWaypoint() {
      var wp = 0;
      for (var i = 0; i < this.current[0]; i++) {
     ...
     */
    currentLeg: function currentLeg() {
     ...
     */
    currentWaypoint: function currentWaypoint() {
      if (this.legs.length < 1) return null;else return this.legs[this.current[0]].waypoints[this.current[1]];
    },
    ...
    fixes: function fixes() {
      return _jquery2.default.map(this.waypoints(), function (w) {
        return w.fix;
      });
    },
     ...
     */
    my_aircraft: function my_aircraft() {
     ...
     */
    waypoint: function waypoint(pos) {
    ...
    waypoints: function waypoints() {
      return _jquery2.default.map(this.legs, function (v) {
        return v.waypoints;
      });
    },

    atLastWaypoint: function atLastWaypoint() {
...
},{"./Leg":24,"./Waypoint":25,"fiber":1,"jquery":2}],22:[function(require,module,exports){
'use strict';
 ...
 * @extends Fiber
 */
var Aircraft = _fiber2.default.extend(function () {
  var _ref;
  ...
  return _ref = {
    init: function init(options) {
      ...
      if (options.category == "arrival") this.setArrivalRunway(airport_get().runway);else if (options.category == "departure") this.setDepartureRunway(airport_get().runway);
    ...
    setArrivalWaypoints: function setArrivalWaypoints(waypoints) {
     ...
     */
    createStrip: function createStrip() {
      this.html = (0, _jquery2.default)("<li class='strip'></li>");
      ...
      // Bottom Line Data
      if (["H", "U"].indexOf(this.model.weightclass) > -1) this.html.append("<span class='aircraft'>" + "H/" + this.model.icao + "</span>");else this.html.append("<span class='aircraft'>" + this.model.icao + "</span>");
      this.html.append("<span class='destination'>" + this.destination + "</span>");
      this.html.append("<span class='speed'>???</span>");

      // Initial Styling
      if (this.category == "departure") this.html.addClass('departure');else this.html.addClass('arrival');
      ...
      if (this.category == "arrival") this.html.hide(0);else if (this.category == "departure") this.inside_ctr = true;
    },
    // Called when the aircraft crosses the center boundary
    crossBoundary: function crossBoundary(inbound) {
    ...
    matchCallsign: function matchCallsign(callsign) {
    ...
    getAirline: function getAirline() {
      return airline_get(this.airline);
    },
    getRadioCallsign: function getRadioCallsign(condensed) {
      var heavy = "";
      ...
      if (cs == "November") cs += " " + radio_spellOut(callsign) + heavy;else cs += " " + groupNumbers(callsign, this.airline) + heavy;
      return cs;
    },
    getClimbRate: function getClimbRate() {
      var a = this.altitude;
      var r = this.model.rate.climb;
      var c = this.model.ceiling;
      if (this.model.engines.type == "J") var serviceCeilingClimbRate = 500;else var serviceCeilingClimbRate = 100;
  ...
  }, _defineProperty(_ref, 'getClimbRate', function getClimbRate() {
    var a = this.altitude;
    var r = this.model.rate.climb;
    var c = this.model.ceiling;
    if (this.model.engines.type == "J") var serviceCeilingClimbRate = 500;else var serviceCeilingClimbRate = 100;
    ...
    return cr_current;
  }), _defineProperty(_ref, 'hideStrip', function hideStrip() {
    this.html.hide(600);
  }), _defineProperty(_ref, 'COMMANDS', {
    abort: 'runAbort',
    ...
    takeoff: 'runTakeoff',
    taxi: 'runTaxi'
  }), _defineProperty(_ref, 'runCommands', function runCommands(commands) {
    if (!this.inside_ctr) return true;
    ...
    if (response.length >= 1) {
      ...
      var r_log = function f() {
        var x = [];for (var i = 0; i < response.length; i++) {
          x.push(response[i].log);
        }return x;
      };
      var r_say = function f() {
        var x = [];for (var i = 0; i < response.length; i++) {
          x.push(response[i].say);
        }return x;
      };
    ...
    return true;
  }), _defineProperty(_ref, 'run', function run(command, data) {
    var call_func;
    ...
    if (!call_func) return ["fail", "not understood"];

    return this[call_func].apply(this, [data]);
  }), _defineProperty(_ref, 'runHeading', function runHeading(data) {
    ...
    if (direction) instruction = "turn " + direction + " heading ";else instruction = "fly heading ";
    if (incremental) var readback = {
      log: "turn " + amount + " degrees " + direction,
      say: "turn " + groupNumbers(amount) + " degrees " + direction };else var readback = {
      log: instruction + heading_to_string(wp.heading),
      say: instruction + radio_heading(heading_to_string(wp.heading)) };
    return ['ok', readback];
  }), _defineProperty(_ref, 'runAltitude', function runAltitude(data) {
    var altitude = data[0];
    var expedite = data[1];
    ...
    if (this.mode == "landing") this.cancelLanding();

    var ceiling = airport_get().ctr_ceiling;
    if (prop.game.option.get('softCeiling') == 'yes') ceiling += 1000;
    ...
    if (expedite) expedite = " and expedite";else expedite = "";
    ...
    return ['ok', readback];
  }), _defineProperty(_ref, 'runClearedAsFiled', function runClearedAsFiled() {
    if (this.fms.clearedAsFiled()) {
      return ['ok', { log: "cleared to destination via the " + airport_get().sids[this.destination].icao + " departure, then as filed" + ". Climb and maintain " + airport_get().initial_alt + ", expect " + this.fms.fp.altitude + " 10 minutes after departure",
        say: "cleared to destination via the " + airport_get().sids[this.destination].name + " departure, then as filed" + ". Climb and maintain " + radio_altitude(airport_get().initial_alt) + ", expect " + radio_altitude(this.fms.fp.altitude) + "," + radio_spellOut(" 10 ") + "minutes after departure" }];
    } else return [true, "unable to clear as filed"];
  }), _defineProperty(_ref, 'runClimbViaSID', function runClimbViaSID() {
    if (!(this.fms.currentLeg().type == "sid")) var fail = true;else if (this.fms.climbViaSID()) return ['ok', { log: "climb via the " + this.fms.currentLeg().route.split('.')[1] + " departure",
  ...
  }), _defineProperty(_ref, 'runDescendViaSTAR', function runDescendViaSTAR() {
    if (this.fms.descendViaSTAR() && this.fms.following.star) return ['ok', { log: "descend via the " + this.fms.following.star + " arrival",
      say: "descend via the " + airport_get().stars[this.fms.following.star].name + " arrival" }];else ui_log(true, this.getCallsign() + ", unable to descend via STAR");
  }), _defineProperty(_ref, 'runSpeed', function runSpeed(data) {
    var speed = data[0];
    if (isNaN(speed)) return ["fail", "speed not understood"];

    this.fms.setAll({ speed: clamp(this.model.speed.min, speed, this.model.speed.max) });
    var readback = {
    ...
    return ["ok", readback];
  }), _defineProperty(_ref, 'runHold', function runHold(data) {
    var dirTurns = data[0];
    ...
    if (holdFix) {
      ...
      if (holdFix != this.fms.currentWaypoint().fix) {
        ...
        this.fms.insertLegHere({ type: "fix", route: "[GPS/RNAV]", waypoints: [new zlsa.atc.Waypoint({ // proceed direct to holding fix
          ...
          }, this.fms), new zlsa.atc.Waypoint({ // then enter the hold
            navmode: "hold",
            speed: this.fms.currentWaypoint().speed,
            altitude: this.fms.currentWaypoint().altitude,
            fix: null,
  ...
  }), _defineProperty(_ref, 'runDirect', function runDirect(data) {
    ...
    if (this.mode == "takeoff") {
      ...
      if (!this.fms.skipToFix(fixname)) return ["fail", fixname + ' is not in our flightplan'];
    }

    return ["ok", "proceed direct " + fixname];
  }), _defineProperty(_ref, 'runFix', function runFix(data) {
    var last_fix, fail;
    var fixes = _jquery2.default.map(data[0], function (fixname) {
      var fix = airport_get().getFix(fixname);
      if (!fix) {
  ...
  }), _defineProperty(_ref, 'runFlyPresentHeading', function runFlyPresentHeading(data) {
    this.cancelFix();
    this.runHeading([null, degrees(this.heading)]);
    return ["ok", "fly present heading"];
  }), _defineProperty(_ref, 'runSayRoute', function runSayRoute(data) {
    return ['ok', { log: 'route: ' + this.fms.fp.route.join(' '), say: "here's our route" }];
  }), _defineProperty(_ref, 'runSID', function runSID(data) {
    var apt = airport_get();
  ...
  }), _defineProperty(_ref, 'runSTAR', function runSTAR(data) {
  ...
  }), _defineProperty(_ref, 'runMoveDataBlock', function runMoveDataBlock(dir) {
  ...
  }), _defineProperty(_ref, 'runRoute', function runRoute(data) {
    ...
    // Build the response
    if (worked) return ['ok', { log: 'rerouting to :' + this.fms.fp.route.join(' '), say: "rerouting as requested" }];else return ['fail', { log: 'your route "' + data + '" is invalid!', say: 'that route is invalid!' }];
  }), _defineProperty(_ref, 'runReroute', function runReroute(data) {
    data = data[0].toUpperCase(); // capitalize everything
    var worked = true;
    var route = this.fms.formatRoute(data);
    ...
    // Build the response
    if (worked) return ['ok', { log: 'rerouting to: ' + this.fms.fp.route.join(' '), say: "rerouting as requested" }];else return ['fail', { log: 'your route "' + data + '" is invalid!', say: 'that route is invalid!' }];
  }), _defineProperty(_ref, 'runTaxi', function runTaxi(data) {
    ...
    if (data[0]) {
      if (airport_get().getRunway(data[0].toUpperCase())) this.setDepartureRunway(data[0].toUpperCase());else return ["fail", "no runway " + data[0].toUpperCase()];
    }
    ...
    return ["ok", readback];
  }), _defineProperty(_ref, 'runTakeoff', function runTakeoff(data) {
    if (this.category != "departure") return ["fail", "inbound"];
  ...
  }), _defineProperty(_ref, 'runLanding', function runLanding(data) {
    var variant = data[0];
    ...
    return ["ok", readback];
  }), _defineProperty(_ref, 'runAbort', function runAbort(data) {
    if (this.mode == "taxi") {
      this.mode = "apron";
      this.taxi_start = 0;
      ...
      this.cancelFix();
      if (this.category == "arrival") return ["ok", "fly present heading, vector to final approach course"];else if (this.category == "departure") return ["ok", "fly present heading, vector for entrail spacing"];
    } else {
      //modes "apron", "takeoff", ("cruise" for some navmodes)
      return ["fail", "unable to abort"];
    }
  }), _defineProperty(_ref, 'runDebug', function runDebug() {
    window.aircraft = this;
    return ["ok", { log: "in the console, look at the variable &lsquo;aircraft&rsquo;", say: "" }];
  }), _defineProperty(_ref, 'runDelete', function runDelete() {
    aircraft_remove(this);
  }), _defineProperty(_ref, 'cancelFix', function cancelFix() {
    ...
    return false;
  }), _defineProperty(_ref, 'cancelLanding', function cancelLanding() {
  ...
  }), _defineProperty(_ref, 'pushHistory', function pushHistory() {
    this.history.push([this.position[0], this.position[1]]);
    if (this.history.length > 10) {
      this.history.splice(0, this.history.length - 10);
    }
  }), _defineProperty(_ref, 'moveForward', function moveForward() {
    this.mode = "taxi";
    this.taxi_next = true;
  }), _defineProperty(_ref, 'isEstablished', function isEstablished() {
    if (this.mode != "landing") return false;
    return this.approachOffset <= 0.048; // 160 feet or 48 meters
  }), _defineProperty(_ref, 'isLanded', function isLanded() {
    var runway = airport_get().getRunway(this.rwy_arr);
    if (runway == null) {
    ...
    if (this.altitude - runway.elevation < 5) return true;
    return false;
  }), _defineProperty(_ref, 'isPrecisionGuided', function isPrecisionGuided() {
  ...
  }), _defineProperty(_ref, 'isStopped', function isStopped() {
    if (this.isLanded() && this.speed < 5) return true;
  }), _defineProperty(_ref, 'isTaxiing', function isTaxiing() {
    if (this.mode == "apron" || this.mode == "taxi" || this.mode == "waiting") {
      return true;
    }
    return false;
  }), _defineProperty(_ref, 'isTakeoff', function isTakeoff() {
    if (this.isTaxiing() || this.mode == "takeoff") {
      return true;
    }
    return false;
  }), _defineProperty(_ref, 'isVisible', function isVisible() {
    if (this.mode == "apron" || this.mode == "taxi") return false; // hide aircraft on twys
        ...
        var runway = airport_get().getRunway(this.rwy_dep);
        var waiting = runway.inQueue(this);
        if (this.mode == "waiting" && waiting == 0) return true;else return false;
      } else return true;
  }), _defineProperty(_ref, 'getWind', function getWind() {
  ...
  }), _defineProperty(_ref, 'radioCall', function radioCall(msg, sectorType, alert) {
    if (this.projected) return;
    var call = "",
    ...
    if (alert) ui_log(true, airport_get().radio[sectorType] + ", " + callsign_L + " " + msg);else ui_log(airport_get().radio[sectorType] + ", " + callsign_L + " " + msg);
    speech_say([{ type: "text", content: airport_get().radio[sectorType] + ", " + callsign_S + " " + msg }]);
  }), _defineProperty(_ref, 'callUp', function callUp() {
    if (this.category == "arrival") {
  ...
  }), _defineProperty(_ref, 'scoreWind', function scoreWind(action) {
    var score = 0;
    ...
    return score;
  }), _defineProperty(_ref, 'showStrip', function showStrip() {
    this.html.detach();
    ...
    this.html.show();
    (0, _jquery2.default)("#strips").scrollTop(scrollPos + 45); // shift scroll down one strip's height
  }), _defineProperty(_ref, 'updateTarget', function updateTarget() {
    var airport = airport_get();
    var runway = null;
    var offset = null;
    ...
    if (this.fms.currentWaypoint().altitude > 0) this.fms.setCurrent({ altitude: Math.max(1000, this.fms.currentWaypoint().altitude) });
    if (this.fms.currentWaypoint().navmode == "rwy") {
      airport = airport_get();
      ...
      if (this.mode == "landing") this.target.altitude = glideslope_altitude;

      var ils = runway.ils.loc_maxDist;
      if (!runway.ils.enabled || !ils) ils = 40;

      // lock  ILS if at the right angle and altitude
      if (abs(this.altitude - glideslope_altitude) < glideslope_window && abs(offset_angle) < radians(10) && offset[1] < ils) {
        if (abs(offset[0]) < 0.05 && this.mode != "landing") {
          this.mode = "landing";
          if (!this.projected && abs(angle_offset(this.fms.currentWaypoint().heading, radians(parseInt(this.rwy_arr.substr(0, 2)) * 10))) > radians(30)) {
        ...
        if (dist_to_localizer <= turning_radius || dist_to_localizer < 0.5) {
          ...
          if (offset[1] > 0.2 && abs(offset[0]) > 0.003) this.target.heading = clamp(radians(-30), -12 * offset_angle, radians(30)) + angle;else this.target.heading = angle;
    ...
    if (this.mode == "taxi") {
      ...
      var runway = airport_get().getRunway(this.rwy_dep);
      // Altitude Control
      if (this.speed < this.model.speed.min) this.target.altitude = runway.elevation;else this.target.altitude = this.fms.currentWaypoint().altitude;

      // Heading Control
      var rwyHdg = airport_get().getRunway(this.rwy_dep).angle;
      if (this.altitude - runway.elevation < 400) this.target.heading = rwyHdg;else {
  ...
  }), _defineProperty(_ref, 'updatePhysics', function updatePhysics() {
    ...
    if (this.hit) {
      ...
      if (distance) {
        ...
        if (abs(offset) < abs(distance)) this.altitude = this.target.altitude;else this.altitude += distance;
      }
      ...
      if (difference) {
        ...
        if (abs(offset) < abs(difference)) this.speed = this.target.speed;else this.speed += difference;
      }
    }
    ...
    if (prop.game.option.get('simplifySpeeds') == 'no') {
      ...
      if (this.isLanded()) {
        ...
        vector = vadd(vscale(vturn(wind.angle + Math.PI), wind.speed * 0.000514444 * game_delta()), vscale(vturn(angle + crab_angle), scaleSpeed));
      }
      this.ds = vlen(vector);
  ...
  }), _defineProperty(_ref, 'updateWarning', function updateWarning() {
  ...
  }), _defineProperty(_ref, 'updateStrip', function updateStrip() {
    if (this.projected) return;
          ...
          // When in normal flight...
              ...
              if (wp.navmode == "fix") {
                ...
                if (this.fms.following.star) {
                  ...
                  if (this.fms.currentWaypoint().fixRestrictions.altitude) altitude.addClass("followingSTAR");
                  destination.text(this.fms.following.star + '.' + airport_get().icao);
                  destination.addClass("followingSTAR");
                  if (this.fms.currentWaypoint().fixRestrictions.speed) speed.addClass("followingSTAR");
                }
              } else if (wp.navmode == "hold") {
                heading.text("holding");
                heading.addClass("hold");
              } else if (wp.navmode == "rwy") {
  ...
  }), _defineProperty(_ref, 'updateAuto', function updateAuto() {}), _defineProperty(_ref, 'update', function update() {
    if (prop.aircraft.auto.enabled) {
      this.updateAuto();
    }
    this.updateTarget();
    this.updatePhysics();
  }), _defineProperty(_ref, 'addConflict', function addConflict(conflict, other) {
    this.conflicts[other.getCallsign()] = conflict;
  }), _defineProperty(_ref, 'checkConflict', function checkConflict(other) {
    if (this.conflicts[other.getCallsign()]) {
      this.conflicts[other.getCallsign()].update();
      return true;
    }
    return false;
  }), _defineProperty(_ref, 'hasAlerts', function hasAlerts() {
    var a = [false, false];
    var c = null;
    for (var i in this.conflicts) {
    ...
    return a;
  }), _defineProperty(_ref, 'removeConflict', function removeConflict(other) {
...
},{"fiber":1,"jquery":2}],23:[function(require,module,exports){
'use strict';
...
},{"./AircraftInstanceModel":22,"fiber":1,"jquery":2}],24:[function(require,module,exports){
'use strict';
...
},{"./Waypoint":25,"fiber":1}],25:[function(require,module,exports){
'use strict';
...
},{"fiber":1}],26:[function(require,module,exports){
'use strict';
...
window.zlsa.atc.Conflict = _AircraftConflict2.default;
var Model = _AircraftModel2.default;
window.zlsa.atc.AircraftFlightManagementSystem = _AircraftFlightManagementSystem2.default;
var Aircraft = _AircraftInstanceModel2.default;
...
window.aircraft_callsign_new = function aircraft_callsign_new(airline) {
  ...
  while (true) {
    ...
    if (prop.aircraft.callsigns.indexOf(callsign) == -1) break;
  }
  ...
  for (var i = 0; i < prop.aircraft.list.length; i++) {
    ...
    InnerLoop: for (var j = i + 1; j < prop.aircraft.list.length; j++) {
      ...
      if (dx > 14.816 || dy > 14.816) {
        ...
        new _AircraftConflict2.default(that, other);
      }
    }
  }
...
window.aircraft_get_by_callsign = function aircraft_get_by_callsign(callsign) {
  ...
  for (var i = 0; i < prop.aircraft.list.length; i++) {
    if (prop.aircraft.list[i].callsign == callsign.toLowerCase()) return prop.aircraft.list[i];
  }return null;
};
...
window.aircraft_get_eid_by_callsign = function aircraft_get_eid_by_callsign(callsign) {
  for (var i = 0; i < prop.aircraft.list.length; i++) {
    if (prop.aircraft.list[i].callsign == callsign.toLowerCase()) return prop.aircraft.list[i].eid;
  }return null;
};
...
},{"../utilities/unitConverters":43,"./AircraftConflict":20,"./AircraftFlightManagementSystem":21,"./AircraftInstanceModel":22,"./AircraftModel":23}],27:[function(require,module,exports){
"use strict";
...
},{"./utilities/unitConverters":43}],29:[function(require,module,exports){
'use strict';
...
},{"./utilities/timeHelpers":42}],30:[function(require,module,exports){
'use strict';
...
},{"fiber":1}],31:[function(require,module,exports){
'use strict';
...
},{"./utilities/timeHelpers":42,"./utilities/unitConverters":43}],32:[function(require,module,exports){
'use strict';
...
},{"./utilities/timeHelpers":42}],36:[function(require,module,exports){
'use strict';
...
},{"./aircraft/aircraft":26,"./airline":27,"./airport":28,"./animation":29,"./base":30,"./canvas":31,"./game":32,"./get":33,"./input":34,"./load":35,"./parser":37,"./speech":38,"./tutorial":39,"./ui":40,"./util":41,"./utilities/timeHelpers":42,"fiber":1,"jquery":2,"pegjs":16}],37:[function(require,module,exports){
"use strict";
...
},{"./utilities/timeHelpers":42}],40:[function(require,module,exports){
'use strict';
...
},{"./utilities/timeHelpers":42,"./utilities/unitConverters":43}],42:[function(require,module,exports){
"use strict";
