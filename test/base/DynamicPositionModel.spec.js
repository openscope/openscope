...
ava('sets internal properties when provided valid parameters', t => {
    const result = new DynamicPositionModel(LAT_LONG_MOCK, airportPositionFixtureKLAS);
...
ava('get relativePosition() returns [0, 0] if no reference position is provided', t => {
    const result = new DynamicPositionModel(LAT_LONG_MOCK, null);
...
ava('.bearingFromPosition() returns the correct bearing between two DynamicPositionModel instances', t => {
    const position1 = new DynamicPositionModel(LAT_LONG_MOCK, airportPositionFixtureKLAS);
    const position2 = new DynamicPositionModel(LAT_LONG_MOCK_2, airportPositionFixtureKLAS);
