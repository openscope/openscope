...
const Aircraft = Fiber.extend(function() {
    return {
        ...
        runAltitude: function(data) {
            ...
            const readback = {};
            readback.log = radio_trend('altitude', this.altitude, this.fms.currentWaypoint().altitude) + ' ' + this.fms.currentWaypoint().altitude + expedite,
            readback.say = radio_trend('altitude', this.altitude, this.fms.currentWaypoint().altitude) + ' ' + radio_altitude(this.fms.currentWaypoint().altitude) + expedite
    ...
    runDirect: function(data) {
      ...
      if (this.mode == FLIGHT_MODES.TAKEOFF) {  // remove intermediate fixes
    ...
    runMoveDataBlock: function(dir) {
      let positions = {8:360,9:45,6:90,3:135,2:180,1:225,4:270,7:315,5:'ctr'};
    ...
    runRoute: function(data) {
      ...
      let worked = true;
      let route = this.fms.formatRoute(data);
      if (worked && route) {
    ...
    runTaxi: function(data) {
      if (this.category !== FLIGHT_CATEGORY.DEPARTURE) {return ["fail", "inbound"];}

      if (this.mode === FLIGHT_MODES.TAXI) { return ["fail", "already taxiing to " + radio_runway(this.rwy_dep)];}

      if (this.mode === FLIGHT_MODES.WAITING) { return ["fail", "already waiting"];}

      if (this.mode !== FLIGHT_MODES.APRON) { return ["fail", "wrong mode"];}
      ...
      let runway = airport_get().getRunway(this.rwy_dep);
    ...
    runTakeoff: function(data) {
      ...
      if (runway.removeQueue(this)) {
        ...
        let wind = airport_get().getWind();
        let wind_dir = round(radiansToDegrees(wind.angle));
        let readback = {
        ...
        let waiting = runway.inQueue(this);
    ...
    runLanding: function(data) {
      var variant = data[0];
      let runway = airport_get().getRunway(data[1]);
      ...
      let readback = {log:"cleared ILS runway " + this.rwy_arr + " approach",
    ...
    runAbort: function(data) {
      if (this.mode == FLIGHT_MODES.TAXI) {
        ...
        this.cancelFix();
        if (this.category == FLIGHT_CATEGORY.ARRIVAL) return ["ok", "fly present heading, vector to final approach course"];
    ...
    cancelFix: function() {
      if (this.fms.currentWaypoint().navmode == WAYPOINT_NAV_MADE.FIX) {
        let curr = this.fms.currentWaypoint();
    ...
    parse: function(data) {
      let keys = 'position model airline callsign category heading altitude speed'.split(' ');
    ...
    isVisible: function() {
      if (this.mode == FLIGHT_MODES.APRON || this.mode == FLIGHT_MODES.TAXI) {
        ...
        let runway = airport_get().getRunway(this.rwy_dep);
        let waiting = runway.inQueue(this);
        if (this.mode == FLIGHT_MODES.WAITING && waiting == 0) return true;
        else return false;
      }
    ...
    getWind: function() {
      ...
      let wind = airport.wind;
      let runway = airport.getRunway(this.rwy_dep);
      let angle =  abs(angle_offset(runway.angle, wind.angle));
    ...
    radioCall: function(msg, sectorType, alert) {
      ...
      let call = '';
      let callsign_L = this.getCallsign();
      let callsign_S = this.getRadioCallsign();;
    ...
    callUp: function() {
      ...
      if (this.category == FLIGHT_CATEGORY.ARRIVAL) {
        let altdiff = this.altitude - this.fms.currentWaypoint().altitude;
        let alt = digits_decimal(this.altitude, -2);
    ...
    updateStrip: function() {
      ...
      let heading  = this.html.find('.heading');
      let altitude = this.html.find('.altitude');
      let destination = this.html.find('.destination');
      let speed = this.html.find('.speed');
      let wp = this.fms.currentWaypoint();
