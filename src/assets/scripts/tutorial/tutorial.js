/* eslint-disable camelcase, no-underscore-dangle, no-mixed-operators, func-names, object-shorthand, no-undef, indent */
import $ from 'jquery';
...
function tutorial_open() {
    ...
    $tutorial.addClass(SELECTORS.CLASSNAMES.OPEN);
    $toggleTutorial.addClass(SELECTORS.CLASSNAMES.ACTIVE);
    $toggleTutorial.prop('title', 'Close tutorial');
...
function tutorial_close() {
  ...
  $tutorial.removeClass(SELECTORS.CLASSNAMES.OPEN);
  $toggleTutorial.removeClass(SELECTORS.CLASSNAMES.ACTIVE);
  $toggleTutorial.prop('title', 'Open tutorial');
...
function tutorial_update_content() {
    ...
    if (step.side === SELECTORS.CLASSNAMES.LEFT) {
        $tutorial.addClass(SELECTORS.CLASSNAMES.LEFT);
    } else if (step.side === SELECTORS.CLASSNAMES.RIGHT) {
        $tutorial.addClass(SELECTORS.CLASSNAMES.RIGHT);
    }
...
function tutorial_move() {
    const step = tutorial_get();
    const padding = [30, 10];
    const left = step.position[0] * ($(window).width() - $tutorial.outerWidth()  - padding[0]);
...
function tutorial_init() {
    ...
    $tutorial = $(SELECTORS.DOM_SELECTORS.TUTORIAL);
    $toggleTutorial = $(SELECTORS.DOM_SELECTORS.TOGGLE_TUTORIAL);
}
