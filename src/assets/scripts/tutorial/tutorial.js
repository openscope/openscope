 ...
 */
function tutorial_get(step = null) {
    if (!step) {
 ...
 */
function tutorial_move() {
    const step = tutorial_get();
    const padding = [30, 10];
    const left = step.position[0] * ($(window).width() - $(SELECTORS.DOM_SELECTORS.TUTORIAL).outerWidth() - padding[0]);
    let top = step.position[1] * ($(window).height());
    top -= ($(SELECTORS.DOM_SELECTORS.TUTORIAL).outerHeight() - padding[1]);

//  left += step.padding[0];
//  top  += step.padding[1];

    $(SELECTORS.DOM_SELECTORS.TUTORIAL).offset({
        top: round(top),
        left: round(left)
    });
}
...
function tutorial_update_content() {
    ...
    $(SELECTORS.DOM_SELECTORS.TUTORIAL).find('h1').html(step.title);
    $(SELECTORS.DOM_SELECTORS.TUTORIAL).find('main').html(step.getText());
    $(SELECTORS.DOM_SELECTORS.TUTORIAL).removeClass('left right');

    if (step.side === SELECTORS.CLASSNAMES.LEFT) {
        $(SELECTORS.DOM_SELECTORS.TUTORIAL).addClass(SELECTORS.CLASSNAMES.LEFT);
    } else if (step.side === SELECTORS.CLASSNAMES.RIGHT) {
        $(SELECTORS.DOM_SELECTORS.TUTORIAL).addClass(SELECTORS.CLASSNAMES.RIGHT);
    }
...
function tutorial_open() {
    ...
    $(SELECTORS.DOM_SELECTORS.TUTORIAL).addClass(SELECTORS.CLASSNAMES.OPEN);
    $(SELECTORS.DOM_SELECTORS.TOGGLE_TUTORIAL).addClass(SELECTORS.CLASSNAMES.ACTIVE);
    $(SELECTORS.DOM_SELECTORS.TOGGLE_TUTORIAL).prop('title', 'Close tutorial');
...
function tutorial_close() {
    ...
    $(SELECTORS.DOM_SELECTORS.TUTORIAL).removeClass(SELECTORS.CLASSNAMES.OPEN);
    $(SELECTORS.DOM_SELECTORS.TOGGLE_TUTORIAL).removeClass(SELECTORS.CLASSNAMES.ACTIVE);
    $(SELECTORS.DOM_SELECTORS.TOGGLE_TUTORIAL).prop('title', 'Open tutorial');
...
function tutorial_init() {
    ...
    prop.tutorial.html.find(SELECTORS.DOM_SELECTORS.PREV).click(tutorial_prev);
    prop.tutorial.html.find(SELECTORS.DOM_SELECTORS.NEXT).click(tutorial_next);
...
// TODO: temporarily exposed here so as not to break the previous interface.
window.tutorial_init_pre = tutorial_init_pre;
window.tutorial_init = tutorial_init;
window.tutorial_toggle = tutorial_toggle;
