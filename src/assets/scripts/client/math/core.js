...
export function calculateNormalDistributedNumber(average) {
    // This gets the average of four (psuedo)random numbers, which makes it a rough bell curve.
    // This means that the average would be around 0.5, so we only divide by two instead of four (making the average about 1).
    const initialNumber = ((Math.random() + Math.random() + Math.random() + Math.random()) / 2);
